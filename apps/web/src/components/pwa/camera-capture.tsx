...
interface CameraCaptureProps {
  /** Rótulo do passo atual */
  label: string;
  /** Dica visual para o usuário */
  hint: string;
  /** Chamado com o File capturado/selecionado */
  onCapture: (file: File) => void;
  /** Se true, desabilita a interação */
  disabled?: boolean;
  /** Número do passo atual (1-based) */
  step?: number;
}
...
export function CameraCapture({ label, hint, onCapture, disabled, step }: CameraCaptureProps) {
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      // Câmera negada ou não disponível — cair no fallback de upload
      setCameraError(true);
    ...
    return (
      <div className="overflow-hidden rounded-2xl bg-black">
        ...
        <div className="flex gap-3 p-4">
          <button
            onClick={retakePhoto}
            className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-white/20 py-3 font-semibold text-white hover:bg-white/10"
          >
          ...
          <button
            ...
            className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-green-500 py-3 font-semibold text-white hover:bg-green-600"
          >
    ...
    return (
      <div className="relative overflow-hidden rounded-2xl bg-black">
        ...
        <div className="absolute bottom-6 left-0 right-0 flex items-center justify-center gap-8 px-8">
          ...
          <button
            onClick={takePhoto}
            className="flex h-20 w-20 items-center justify-center rounded-full bg-white shadow-lg active:scale-95 transition-transform"
          >
    ...
    <div className={cn('rounded-2xl bg-white p-6 shadow-sm', disabled && 'opacity-50')}>
      ...
      <div className="mt-5 flex gap-3">
        ...
        <button
          onClick={startCamera}
          disabled={disabled}
          className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-brand-gold py-3 font-semibold text-white hover:bg-brand-gold-dark active:scale-95 transition-transform disabled:opacity-50"
        >
        ...
        <button
          ...
          disabled={disabled}
          className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-gray-200 py-3 font-semibold text-gray-700 hover:bg-gray-50 active:scale-95 transition-transform disabled:opacity-50"
        >
      ...
      {/* Input file oculto (fallback + galeria) */}
      <input
        ref={fileInputRef}
        type="file"
        accept="image/*"
